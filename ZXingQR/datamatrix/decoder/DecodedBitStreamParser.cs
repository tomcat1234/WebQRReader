...
namespace com.google.zxing.datamatrix.decoder
{
	...
	sealed class DecodedBitStreamParser
	{
		...
		private static void  parseTwoBytes(int firstByte, int secondByte, int[] result)
		{
			...
			int temp = Math.Floor(fullBitValue / 1600);
			result[0] = temp;
		...
		private static void  decodeBase256Segment(BitSource bits, StringBuilder result, ArrayList byteSegments)
		{
			...
			byteSegments.Add(SupportClass.ToByteArray1(bytes));
			try
			{
				...
				result.Append(Encoding.GetEncoding("ISO8859_1").GetString(SupportClass.ToByteArray1(bytes)));
			}
			catch (Exception uee)
			{
				//UPGRADE_TODO: The equivalent in .NET for method 'java.lang.Throwable.toString' may return a different value. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1043'"
				throw new Exception("SystemExpection: Platform does not support required encoding: " + uee);
			}
		}
