...
namespace com.google.zxing.common.reedsolomon
{
	...
	public sealed class ReedSolomonEncoder
	{
		...
		public ReedSolomonEncoder(GF256 field)
		{
			...
			{
				throw new Exception("Only QR Code is supported at this time");
			}
			this.field = field;
		...
		private GF256Poly buildGenerator(int degree)
		{
			if (degree >= cachedGenerators.Count)
			{
				...
				{
					GF256Poly nextGenerator = lastGenerator.multiply1(new GF256Poly(field, new int[]{1, field.exp(d - 1)}));
		...
		public void  encode(int[] toEncode, int ecBytes)
		{
			...
			ArrayExtend.Copy(toEncode, 0, infoCoefficients, 0, dataBytes);
			...
			ArrayExtend.Copy(coefficients, 0, toEncode, dataBytes + numZeroCoefficients, coefficients.Length);
		}
	}
}