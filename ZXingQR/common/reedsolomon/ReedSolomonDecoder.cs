	...
	public sealed class ReedSolomonDecoder
	{
		...
		public void  decode(int[] received, int twoS)
		{
			...
			for (int i = 0; i < errorLocations.Length; i++)
			{
				...
				if (position < 0)
				{
					throw new Exception("ReedSolomonException: Bad error location");
				}
		...
		private GF256Poly[] runEuclideanAlgorithm(GF256Poly a, GF256Poly b, int R)
		{
			...
			while (r.Degree >= Math.Floor(R / 2))
			{
				...
				if (rLast.Zero)
				{
					...
					throw new Exception("ReedSolomonException: r_{i-1} was zero");
				}
				r = rLastLast;
			...
			{
				throw new Exception("ReedSolomonException: sigmaTilde(0) was zero");
			}
			
			int inverse = field.inverse(sigmaTildeAtZero);
			GF256Poly sigma = t.multiply2(inverse);
			GF256Poly omega = r.multiply2(inverse);
		...
		private int[] findErrorLocations(GF256Poly errorLocator)
		{
			...
			if (e != numErrors)
			{
				throw new Exception("ReedSolomonException: Error locator degree does not match number of roots");
			}
			return result;
		}
