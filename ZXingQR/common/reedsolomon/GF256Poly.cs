...
namespace com.google.zxing.common.reedsolomon
{
	...
	internal sealed class GF256Poly
	{
		...
		internal GF256Poly(GF256 field, int[] coefficients)
		{
			...
			if (coefficientsLength > 1 && coefficients[0] == 0)
			{
				...
				else
				{
					...
					ArrayExtend.Copy(coefficients, firstNonZero, this.coefficients, 0, this.coefficients.Length);
				}
			}
			else
			{
		...
		internal GF256Poly addOrSubtract(GF256Poly other)
		{
			...
			{
				throw new Exception("GF256Polys do not have same GF256 field");
			}
			if (Zero)
			{
				return other;
			...
			ArrayExtend.Copy(largerCoefficients, 0, sumDiff, 0, lengthDiff);
		...
		internal GF256Poly multiply1(GF256Poly other)
		{
			if (!(field == other.field))
			{
				throw new Exception("GF256Polys do not have same GF256 field");
		...
		internal GF256Poly multiply2(int scalar)
		{
			if (scalar == 0)
			{
				return field.Zero;
