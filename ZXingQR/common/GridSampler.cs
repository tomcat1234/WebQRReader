...
namespace com.google.zxing.common
{
	...
	public abstract class GridSampler
	{
		...
		/// </summary>
		public abstract BitMatrix sampleGrid1(BitMatrix image, int dimension, float p1ToX, float p1ToY, float p2ToX, float p2ToY, float p3ToX, float p3ToY, float p4ToX, float p4ToY, float p1FromX, float p1FromY, float p2FromX, float p2FromY, float p3FromX, float p3FromY, float p4FromX, float p4FromY);
		
		public virtual BitMatrix sampleGrid2(BitMatrix image, int dimension, PerspectiveTransform transform)
		{
		...
		protected internal static void  checkAndNudgePoints(BitMatrix image, float[] points)
		{
			...
			for (int offset = 0; offset < points.Length && nudged; offset += 2)
			{
				...
				int x = (int) Math.Floor(points[offset]);
				//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
				int y = (int) Math.Floor(points[offset + 1]);
