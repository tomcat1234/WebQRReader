...
using ECBlocks = com.google.zxing.qrcode.decoder.ECBlocks;
...
namespace com.google.zxing.qrcode.encoder
{
	...
	public sealed class Encoder
	{
		...
		/// </summary>
		public static void  encode1(String content, ErrorCorrectionLevel ecLevel, QRCode qrCode)
		{
			encode2(content, ecLevel, null, qrCode);
		}
		
		public static void  encode2(String content, ErrorCorrectionLevel ecLevel, Dictionary<object, object> hints, QRCode qrCode)
		{
			...
			Mode mode = chooseMode2(content, encoding);
			...
			if (!qrCode.Valid)
			{
				throw new Exception("WriterException: Invalid QR code: " + qrCode.ToString());
			}
		}
		...
		public static Mode chooseMode1(String content)
		{
			return chooseMode2(content, null);
		}
		...
		private static bool isOnlyDoubleByteKanji(String content)
		{
			...
			try
			{
				//UPGRADE_TODO: Method 'java.lang.String.getBytes' was converted to 'Encoding.GetEncoding(string).GetBytes(string)' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javalangStringgetBytes_javalangString'"
		...
		private static int chooseMaskPattern(BitVector bits, ErrorCorrectionLevel ecLevel, int version, ByteMatrix matrix)
		{
			...
			int minPenalty = Int32Extend.MaxValue; // Lower penalty is better.
		...
		private static void  initQRCode(int numInputBytes, ErrorCorrectionLevel ecLevel, Mode mode, QRCode qrCode)
		{
			...
			throw new Exception("WriterException: Cannot find proper rs block info (input data too big?)");
		}
		...
		internal static void  terminateBits(int numDataBytes, BitVector bits)
		{
			...
			{
				throw new Exception("WriterException: data bits cannot fit in the QR Code" + bits.size() + " > " + capacity);
			}
			...
			if (bits.size() % 8 != 0)
			{
				throw new Exception("WriterException: Number of bits is not a multiple of 8");
			}
		...
		internal static void  getNumDataBytesAndNumECBytesForBlockID(int numTotalBytes, int numDataBytes, int numRSBlocks, int blockID, int[] numDataBytesInBlock, int[] numECBytesInBlock)
		{
			...
			int numTotalBytesInGroup1 = Math.Floor(numTotalBytes / numRSBlocks);
			// numTotalBytesInGroup2 = 39 + 1 = 40
			int numTotalBytesInGroup2 = numTotalBytesInGroup1 + 1;
			// numDataBytesInGroup1 = 66 / 5 = 13
			int numDataBytesInGroup1 = Math.Floor(numDataBytes / numRSBlocks);
		...
		internal static void  interleaveWithECBytes(BitVector bits, int numTotalBytes, int numDataBytes, int numRSBlocks, BitVector result)
		{
			...
			for (int i = 0; i < numRSBlocks; ++i)
			{
				...
				ByteArray dataBytes = new ByteArray(0);
				dataBytes.set_Renamed2(bits.Array, dataBytesOffset, numDataBytesInBlock[0]);
			...
			{
				// Should be same.
				throw new Exception("WriterException: Interleaving error: " + numTotalBytes + " and " + result.sizeInBytes() + " differ.");
			}
		}
		...
		internal static void  appendBytes(String content, Mode mode, BitVector bits, String encoding)
		{
			...
			{
				throw new Exception("WriterException: Invalid mode: " + mode);
			}
		}
		...
		internal static void  appendAlphanumericBytes(String content, BitVector bits)
		{
			...
			while (i < length)
			{
				int code1 = getAlphanumericCode(content.CharCodeAt(i));
				if (code1 == - 1)
				{
				...
				if (i + 1 < length)
				{
					int code2 = getAlphanumericCode(content.CharCodeAt(i + 1));
					if (code2 == - 1)
					{
						throw new Exception("WriterException");
					}
		...
		internal static void  append8BitBytes(String content, BitVector bits, String encoding)
		{
			...
			catch (Exception uee)
			{
				...
				throw new Exception("WriterException: " + uee.ToString());
			}
