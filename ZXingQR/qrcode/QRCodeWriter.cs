...
namespace com.google.zxing.qrcode
{
	...
	public sealed class QRCodeWriter : Writer
	{
		...
		public ByteMatrix encode1(String contents, BarcodeFormat format, int width, int height, Dictionary<object, object> hints)
		{
			...
			{
				throw new Exception("ArgumentException: Can only encode QR_CODE, but got " + format);
			}
			...
			Encoder.encode2(contents, errorCorrectionLevel, hints, code);
		...
		private static ByteMatrix renderResult(QRCode code, int width, int height)
		{
			...
			int multiple = Math.Min(Math.Floor(outputWidth / qrWidth), Math.Floor(outputHeight / qrHeight));
			// Padding includes both the quiet zone and the extra white pixels to accommodate the requested
			// dimensions. For example, if input is 25x25 the QR will be 33x33 including the quiet zone.
			// If the requested size is 200x160, the multiple will be 4, for a QR of 132x132. These will
			// handle all the padding from 100x100 (the actual QR) up to 200x160.
			int leftPadding = Math.Floor((outputWidth - (inputWidth * multiple)) / 2);
			int topPadding = Math.Floor((outputHeight - (inputHeight * multiple)) / 2);
			...
			{
				setRowColor(outputArray[y], (sbyte) SupportClass.Identity1(255));
			}
			...
			for (int y = 0; y < inputHeight; y++)
			{
				...
				{
					row[x] = (sbyte) SupportClass.Identity1(255);
				}
				...
				for (int x = 0; x < inputWidth; x++)
				{
                    ...
                    sbyte value_Renamed = (sbyte)((inputArray[y][x] == 1) ? 0 : SupportClass.Identity1(255));
				...
				for (int z = 0; z < multiple; z++)
				{
					ArrayExtend.Copy(row, 0, outputArray[offset + z], 0, outputWidth);
				}
			}
