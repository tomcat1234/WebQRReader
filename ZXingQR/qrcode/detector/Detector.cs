...
namespace com.google.zxing.qrcode.detector
{
	...
	public class Detector
	{
		...
		public virtual DetectorResult detect1()
		{
			return detect2(null);
		}
		...
		/// </returns>
		/// <throws>  ReaderException if no QR Code can be found </throws>
		public virtual DetectorResult detect2(Dictionary<object, object> hints)
		{
		...
		protected internal virtual DetectorResult processFinderPatternInfo(FinderPatternInfo info)
		{
			...
			if (provisionalVersion.AlignmentPatternCenters.Length > 0)
			{
				...
				//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
				int estAlignmentX = (int) Math.Floor(topLeft.X + correctionToTopLeft * (bottomRightX - topLeft.X));
				//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
				int estAlignmentY = (int) Math.Floor(topLeft.Y + correctionToTopLeft * (bottomRightY - topLeft.Y));
		...
		{
			//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
			float moduleSizeEst1 = sizeOfBlackWhiteBlackRunBothWays((int) Math.Floor(pattern.X), (int) Math.Floor(pattern.Y), (int) Math.Floor(otherPattern.X), (int) Math.Floor(otherPattern.Y));
			//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
			float moduleSizeEst2 = sizeOfBlackWhiteBlackRunBothWays((int) Math.Floor(otherPattern.X), (int) Math.Floor(otherPattern.Y), (int) Math.Floor(pattern.X), (int) Math.Floor(pattern.Y));
			if (SingleExtend.IsNaN(moduleSizeEst1))
			{
				return moduleSizeEst2 / 7.0f;
			}
			if (SingleExtend.IsNaN(moduleSizeEst2))
			{
		...
		private float sizeOfBlackWhiteBlackRunBothWays(int fromX, int fromY, int toX, int toY)
		{
			...
			//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
			int otherToY = (int) Math.Floor(fromY - (toY - fromY) * scale);
		...
		protected internal virtual AlignmentPattern findAlignmentInRegion(float overallEstModuleSize, int estAlignmentX, int estAlignmentY, float allowanceFactor)
		{
			...
			// should be
			//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
			int allowance = (int) Math.Floor(allowanceFactor * overallEstModuleSize);
