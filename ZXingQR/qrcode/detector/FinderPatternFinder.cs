...
namespace com.google.zxing.qrcode.detector
{
	...
	public class FinderPatternFinder
	{
		...
		internal virtual FinderPatternInfo find(Dictionary<object, object> hints)
		{
			...
			int iSkip = Math.Floor((3 * maxI) / (4 * MAX_MODULES));
		...
		protected internal static bool foundPatternCross(int[] stateCount)
		{
			...
			int moduleSize = Math.Floor((totalModuleSize << INTEGER_MATH_SHIFT) / 7);
			int maxVariance = Math.Floor(moduleSize / 2);
		...
		private float crossCheckVertical(int startI, int centerJ, int maxCount, int originalStateCountTotal)
		{
			...
			return foundPatternCross(stateCount)?centerFromEnd(stateCount, i):SingleExtend.NaN;
		}
		...
		private float crossCheckHorizontal(int startJ, int centerI, int maxCount, int originalStateCountTotal)
		{
			...
			return foundPatternCross(stateCount)?centerFromEnd(stateCount, j):SingleExtend.NaN;
		}
		...
		protected internal virtual bool handlePossibleCenter(int[] stateCount, int i, int j)
		{
			...
			//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
			float centerI = crossCheckVertical(i, (int) Math.Floor(centerJ), stateCount[2], stateCountTotal);
			if (!SingleExtend.IsNaN(centerI))
			{
				// Re-cross check
				//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
				centerJ = crossCheckHorizontal((int) Math.Floor(centerJ), (int) Math.Floor(centerI), stateCount[2], stateCountTotal);
				if (!SingleExtend.IsNaN(centerJ))
				{
					//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
	...
	class CenterComparator : Comparator
	{
