	...
	sealed class DecodedBitStreamParser
	{
		...
		private static void  decodeKanjiSegment(BitSource bits, StringBuilder result, int count)
		{
			...
			while (count > 0)
			{
				...
				int assembledTwoBytes = (Math.Floor(twoBytes / 0x0C0) << 8) | (twoBytes % 0x0C0);
			...
			try
			{
				...
				result.Append(Encoding.GetEncoding(SHIFT_JIS).GetString(SupportClass.ToByteArray1(buffer)));
			}
			catch (Exception)
			{
				throw new Exception("ReaderException");
		...
		private static void  decodeByteSegment(BitSource bits, StringBuilder result, int count, CharacterSetECI currentCharacterSetECI, ArrayList byteSegments)
		{
			...
			{
				//UPGRADE_TODO: The differences in the Format  of parameters for constructor 'java.lang.String.String'  may cause compilation errors.  "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1092'"
				result.Append(Encoding.GetEncoding(encoding).GetString(SupportClass.ToByteArray1(readBytes)));
			}
			catch (Exception)
			{
				throw new Exception("ReaderException");
		...
		private static void  decodeNumericSegment(BitSource bits, StringBuilder result, int count)
		{
			...
			while (count >= 3)
			{
				...
				result.Append(ALPHANUMERIC_CHARS[Math.Floor(threeDigitsBits / 100)]);
				result.Append(ALPHANUMERIC_CHARS[Math.Floor(threeDigitsBits / 10) % 10]);
			...
			if (count == 2)
			{
				...
				result.Append(ALPHANUMERIC_CHARS[Math.Floor(twoDigitsBits / 10)]);
		...
		private static String guessEncoding(sbyte[] bytes)
		{
			...
			if (bytes.Length > 3 && bytes[0] == (sbyte) SupportClass.Identity1(0xEF) && bytes[1] == (sbyte) SupportClass.Identity1(0xBB) && bytes[2] == (sbyte) SupportClass.Identity1(0xBF))
			{
				return UTF8;
			}
		...
		private static int parseECIValue(BitSource bits)
		{
			...
			throw new Exception("ArgumentException: Bad ECI bits starting with byte " + firstByte);
		}
