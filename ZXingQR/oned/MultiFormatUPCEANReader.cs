	...
	public sealed class MultiFormatUPCEANReader:OneDReader
	{
		...
		public override Result decodeRow2(int rowNumber, BitArray row, Dictionary<object, object> hints)
		{
			...
			for (int i = 0; i < size; i++)
			{
				...
				try
				{
					result = reader.decodeRow1(rowNumber, row, startGuardPattern, hints);
				}
				catch (Exception e)
				{
				...
				if (result.BarcodeFormat == BarcodeFormat.EAN_13 && result.Text.CharAt(0) == '0')
				{
					return new Result(result.Text.Substr(1), null, result.ResultPoints, BarcodeFormat.UPC_A);
				}
				return result;
			}
