...
namespace com.google.zxing.oned
{
	...
	public abstract class UPCEANWriter : Writer
	{
		...
		private static ByteMatrix renderResult(sbyte[] code, int width, int height)
		{
			...
			int multiple = Math.Floor(outputWidth / fullWidth);
			...
			for (int x = 0; x < inputWidth; x++)
			{
                ...
                sbyte value_Renamed = (code[x] == 1) ? (sbyte)0 : (sbyte)SupportClass.Identity1(255);
			...
			{
				ArrayExtend.Copy(row, 0, outputArray[z], 0, outputWidth);
			}
		...
		protected internal static int appendPattern(sbyte[] target, int pos, int[] pattern, int startColor)
		{
			...
			{
				throw new Exception("ArgumentException: startColor must be either 0 or 1, but got: " + startColor);
			}
		...
		/// <returns> a byte array of horizontal pixels (0 = white, 1 = black) 
		/// </returns>
		public abstract sbyte[] encode2(String contents);
	}
}