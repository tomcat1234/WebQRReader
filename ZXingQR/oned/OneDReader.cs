...
namespace com.google.zxing.oned
{
	...
	public abstract class OneDReader : Reader
	{
			...
			for (int x = 0; x < maxLines; x++)
			{
				...
				for (int attempt = 0; attempt < 2; attempt++)
				{
					if (attempt == 1)
					{
						...
						{
							Dictionary<object, object> newHints = new Dictionary<object, object>(); // Can't use clone() in J2ME
							IEnumerator<object> hintEnum = hints.Keys.GetEnumerator();
					...
					{
						// Look for a barcode
						Result result = decodeRow2(rowNumber, row, hints);
		...
		internal static int patternMatchVariance(int[] counters, int[] pattern, int maxIndividualVariance)
		{
			...
			int unitBarWidth = Math.Floor((total << INTEGER_MATH_SHIFT) / patternLength);
		...
		public abstract Result decodeRow2(int rowNumber, BitArray row, Dictionary<object, object> hints);
	}
}
