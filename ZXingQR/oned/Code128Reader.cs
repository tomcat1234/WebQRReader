...
namespace com.google.zxing.oned
{
	...
	public sealed class Code128Reader:OneDReader
	{
		...
		private static readonly int MAX_AVG_VARIANCE = (int) Math.Floor(PATTERN_MATCH_RESULT_SCALE_FACTOR * 0.25f);
				...
				else
				{
					if (counterPosition == patternLength - 1)
					{
						...
						if (bestMatch >= 0)
						{
							...
							if (row.isRange(Math.Max(0, patternStart - Math.Floor((i - patternStart) / 2)), patternStart, false))
							{
