...
namespace com.google.zxing.oned
{
	...
	public abstract class UPCEANReader:OneDReader
	{
		...
		private static readonly int MAX_AVG_VARIANCE = (int) Math.Floor(PATTERN_MATCH_RESULT_SCALE_FACTOR * 0.42f);
		...
		//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
		private static readonly int MAX_INDIVIDUAL_VARIANCE = (int) Math.Floor(PATTERN_MATCH_RESULT_SCALE_FACTOR * 0.7f);
		...
		{
			decodeRowStringBuffer = new StringBuilder();
		}
		...
		{
			return decodeRow1(rowNumber, row, findStartGuardPattern(row), hints);
		}
		...
		/// </summary>
		public virtual Result decodeRow1(int rowNumber, BitArray row, int[] startGuardRange, Dictionary<object, object> hints)
		{
		...
		private static bool checkStandardUPCEANChecksum(String s)
		{
			...
			{
				int digit = s.CharCodeAt(i) - CharExtend.ToInt32('0');
				if (digit < 0 || digit > 9)
				{
					throw new Exception("ReaderException");
				}
