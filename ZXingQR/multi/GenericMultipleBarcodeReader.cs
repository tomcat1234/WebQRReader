...
namespace com.google.zxing.multi
{
	...
	public sealed class GenericMultipleBarcodeReader : MultipleBarcodeReader
	{
		...
		public Result[] decodeMultiple1(BinaryBitmap image)
		{
			return decodeMultiple2(image, null);
		}
		...
		private void  doDecodeMultiple(BinaryBitmap image, Dictionary<object, object> hints, ArrayList results, int xOffset, int yOffset)
		{
			Result result;
			try
			{
				result = delegate_Renamed.decode2(image, hints);
			}
			catch (Exception e)
			{
			...
			if (minX > MIN_DIMENSION_TO_RECUR)
			{
				...
				doDecodeMultiple(image.crop(0, 0, (int) Math.Floor(minX), Math.Floor(height)), hints, results, xOffset, yOffset);
			}
			// Decode above barcode
			if (minY > MIN_DIMENSION_TO_RECUR)
			{
				//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
				doDecodeMultiple(image.crop(0, 0, width, (int) Math.Floor(minY)), hints, results, xOffset, yOffset);
			}
			// Decode right of barcode
			if (maxX < width - MIN_DIMENSION_TO_RECUR)
			{
				//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
				doDecodeMultiple(image.crop((int) Math.Floor(maxX), 0, width - (int) Math.Floor(maxX), height), hints, results, xOffset + (int) Math.Floor(maxX), yOffset);
			}
			// Decode below barcode
			if (maxY < height - MIN_DIMENSION_TO_RECUR)
			{
				//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
				doDecodeMultiple(image.crop(0, (int) Math.Floor(maxY), width, height - (int) Math.Floor(maxY)), hints, results, xOffset, yOffset + (int) Math.Floor(maxY));
			}
		}
