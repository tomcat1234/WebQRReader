...
namespace com.google.zxing.multi.qrcode.detector
{
	...
	sealed class MultiFinderPatternFinder:FinderPatternFinder
	{
		...
		/// </param>
		internal MultiFinderPatternFinder(BitMatrix image):base(image, null)
		{
		}
		...
		{
			ArrayList possibleCenters = PossibleCenters;
			...
			*/
			...
			ArrayList results = new ArrayList(); // holder for the results
		...
		public FinderPatternInfo[] findMulti(Dictionary<object, object> hints)
		{
			...
			//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
			int iSkip = Math.Floor(maxI / (MAX_MODULES * 4.0f) * 3);
	...
	internal class ModuleSizeComparator : Comparator
	{
