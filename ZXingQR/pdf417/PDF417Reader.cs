...
namespace com.google.zxing.pdf417
{
	...
	public sealed class PDF417Reader : Reader
	{
		...
		public Result decode1(BinaryBitmap image)
		{
		...
		public Result decode2(BinaryBitmap image, Dictionary<object, object> hints)
		{
			...
			if (hints != null && hints.ContainsKey(DecodeHintType.PURE_BARCODE))
			{
				...
				decoderResult = decoder.decode2(bits);
				points = NO_POINTS;
			}
			else
			{
				DetectorResult detectorResult = new Detector(image).detect1();
				decoderResult = decoder.decode2(detectorResult.Bits);
		...
		private static BitMatrix extractPureBits(BinaryBitmap image)
		{
			...
			int dimension = Math.Floor((rowEndOfSymbol - borderWidth) / moduleSize);
