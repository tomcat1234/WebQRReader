	...
	sealed class DecodedBitStreamParser
	{
				...
				switch (subMode)
				{
					...
					case ALPHA: 
						...
						if (subModeCh < 26)
						{
							...
							ch = Int32Extend.ToChar(CharExtend.ToInt32('A') + subModeCh);
						}
						else
						{
							if (subModeCh == 26)
							...
							{
								result.Append(Int32Extend.ToChar(byteCompactionData[i]));
							}
						}
						break;
		...
		private static int byteCompaction(int mode, int[] codewords, int codeIndex, StringBuilder result)
		{
			if (mode == BYTE_COMPACTION_MODE_LATCH)
			{
				...
				for (int i = (Math.Floor(count / 5) * 5); i < count; i++)
				{
			...
			else if (mode == BYTE_COMPACTION_MODE_LATCH_6)
			{
				...
				while ((codeIndex < codewords[0]) && !end)
				{
					...
					if ((count % 5 == 0) && (count > 0))
					{
						...
						for (int j = 0; j < 6; ++j)
						{
							decodedData[5 - j] = Int32Extend.ToChar((int)Math.Floor(value_Renamed % 256));
							value_Renamed >>= 8;
						}
						result.Append(decodedData);
					}
		...
		*/
		private static String decodeBase900toBase10(int[] codewords, int count)
		{
			...
			{
				StringBuilder value_Renamed = multiply(EXP900[count - i - 1], codewords[i]);
				if (accum == null)
				{
			...
			// leading zeros
			for (int i = 0; i < accum.ToString().Length; i++)
			{
				if (accum.ToString().CharAt(i) == '1')
				{
					//result = accum.Substr(i + 1);
					result = accum.ToString().Substr(i + 1);
					break;
				}
			}
			if (result == null)
		...
		private static StringBuilder multiply(String value1, int value2)
		{
			...
			int hundreds = Math.Floor(value2 / 100);
			int tens = Math.Floor(value2 / 10) % 10;
			int ones = value2 % 10;
			// Multiply by ones
			...
			{
				result = add(result.ToString(), (value1 + '0').Substr(1));
			}
			// Multiply by hundreds
			for (int j = 0; j < hundreds; j++)
			{
				result = add(result.ToString(), (value1 + "00").Substr(2));
			}
			return result;
		}
		...
		String result = accum.toString().Substr(1);
		return result;
		}
		*/
	}
