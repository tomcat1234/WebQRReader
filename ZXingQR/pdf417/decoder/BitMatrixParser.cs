	...
	sealed class BitMatrixParser
	{
		...
		internal int processRow(int[] rowCounters, int rowNumber, int rowHeight, int[] codewords, int next)
		{
			...
			for (int i = 0; i < width; i += MODULES_IN_SYMBOL)
			{
				...
				{
					if (rowCounters[i + (MODULES_IN_SYMBOL - 1 - mask)] >= SupportClass.URShift1(rowHeight, 1))
					{
						symbol |= 1L << mask;
					}
				}
				...
				if (ecLevel < 0)
				{
					switch (rowNumber % 3)
					{
						...
						case 2: 
							...
							{
								ecLevel = Math.Floor(((rightColumnECData % 30) - rows % 3) / 3);
							}
							break;
						}
				}
		...
		*/
		...
		private static int findCodewordIndex(long symbol)
		{
			...
			while (first < upto)
			{
				int mid = SupportClass.URShift1((first + upto), 1); // Compute mid point.
